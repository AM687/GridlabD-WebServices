...
// proxy allocator
class controllerProxyAllocator: public staff::IProxyAllocator
{
public:
  controllerProxyAllocator()
  {
    try
    {
      staff::ServiceFactory::Inst().RegisterProxyAllocator(typeid(controller).name(), *this);
    }
    STAFF_CATCH_ALL_DESCR("Failed to register proxy allocator controller");
  }
  ...
  {
    std::auto_ptr<controllerProxy> tpProxy(new controllerProxy);
  ...
  {
    std::auto_ptr<controllerProxy> tpProxy(new controllerProxy);
    tpProxy->Init(sBaseUri + (sServiceName.empty() ? "controller" : sServiceName),
                  sSessionId, sInstanceId);
    return tpProxy.release();
  }
};

controllerProxyAllocator tcontrollerProxyAllocatorInitializer;
...
void controllerProxy::Init(const std::string& sServiceUri, const std::string& sSessionId, const std::string& sInstanceId)
{
  staff::IService::Init("controller", sSessionId, sInstanceId);
  m_sServiceUri = !sServiceUri.empty() ? sServiceUri : "http://10.96.12.46:9090/axis2/services/controller";
...
staff::ServiceClient* controllerProxy::GetClient()
{
  return &m_tClient;
}


int controllerProxy::COinit(double market_init_price, int pstate, int market_period, int control_mode, double psetpoint, double base_setpoint, double heating_base_setpoint, double cooling_base_setpoint)
{
...
int controllerProxy::COpresync(double base_setpoint, double psetpoint, int control_mode)
{
...
double controllerProxy::COsync(double load, double demand, int pstate, double total, double target, double pAvg, double pStd, double base_setpoint, int control_mode, double market_current_frameclearing_price)
{
...
double controllerProxy::COpostsync(int control_mode, double base_setpoint)
{
