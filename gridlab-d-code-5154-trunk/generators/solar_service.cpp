...
std::auto_ptr< ::solar_server > psolar_server(::staff::ServiceFactory::Inst().GetService< ::solar_server >());
...
int solar_service::create(void) 
{
	...
	double solarcreateResult = psolar_server->solarcreate();
	printf("solarcreateResult = %f \n", solarcreateResult);
...
int solar_service::init(OBJECT *parent)
{
    ...
    //int gen_status_v = 0;
    //int panel_type_v = 0;
    int solarinitResult = psolar_server->solarinit(gen_mode_v, gen_status_v, panel_type_v);
	printf("solarinitResult = %d \n", solarinitResult);

	climate_result = solarinitResult;
...
//	 *pLine_I =I_Out;
  //   *pCircuit_V =V_Out;
	double solarsyncResult = psolar_server->solarsync(0, 1);
