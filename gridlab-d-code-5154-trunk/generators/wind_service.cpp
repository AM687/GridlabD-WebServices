...
std::auto_ptr< ::windturbine_server > pwindturbine_server(::staff::ServiceFactory::Inst().GetService< ::windturbine_server >());
...
int wind_service::create(void) 
{
	...
	double windturb_dgcreateResult = pwindturbine_server->windturb_dgcreate();
...
int wind_service::init(OBJECT *parent)
{
	...
	int windturb_dginitResult = pwindturbine_server->windturb_dginit(pLine_I_A_Re, pLine_I_A_Im, pLine_I_B_Re, pLine_I_B_Im, pLine_I_C_Re, pLine_I_C_Im);
...
TIMESTAMP wind_service::sync(TIMESTAMP t0, TIMESTAMP t1) 
{
	...
	double windturb_dgsyncResult = pwindturbine_server->windturb_dgsync(pCircuit_V_A_Re, pCircuit_V_A_Im, pCircuit_V_B_Re, pCircuit_V_B_Im, pCircuit_V_C_Re, pCircuit_V_C_Im);
