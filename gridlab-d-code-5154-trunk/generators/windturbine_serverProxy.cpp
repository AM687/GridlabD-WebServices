...
#include "windturbine_serverProxy.h"
...
// proxy allocator
class windturbine_serverProxyAllocator: public staff::IProxyAllocator
{
public:
  windturbine_serverProxyAllocator()
  {
    try
    {
      staff::ServiceFactory::Inst().RegisterProxyAllocator(typeid(windturbine_server).name(), *this);
    }
    STAFF_CATCH_ALL_DESCR("Failed to register proxy allocator windturbine_server");
  }
  ...
  {
    std::auto_ptr<windturbine_serverProxy> tpProxy(new windturbine_serverProxy);
  ...
  {
    std::auto_ptr<windturbine_serverProxy> tpProxy(new windturbine_serverProxy);
    tpProxy->Init(sBaseUri + (sServiceName.empty() ? "windturbine_server" : sServiceName),
                  sSessionId, sInstanceId);
    return tpProxy.release();
  }
};

windturbine_serverProxyAllocator twindturbine_serverProxyAllocatorInitializer;

// asynch proxies


// service proxy
windturbine_serverProxy::windturbine_serverProxy()
{
}
...
void windturbine_serverProxy::Init(const std::string& sServiceUri, const std::string& sSessionId, const std::string& sInstanceId)
{
  staff::IService::Init("windturbine_server", sSessionId, sInstanceId);
  m_sServiceUri = !sServiceUri.empty() ? sServiceUri : "http://10.96.12.46:9090/axis2/services/windturbine_server";
...
void windturbine_serverProxy::Deinit()
{
...
staff::ServiceClient* windturbine_serverProxy::GetClient()
{
  return &m_tClient;
}


double windturbine_serverProxy::windturb_dgcreate()
{
...
int windturbine_serverProxy::windturb_dginit(double pLine_I_A_Re, double pLine_I_A_Im, double pLine_I_B_Re, double pLine_I_B_Im, double pLine_I_C_Re, double pLine_I_C_Im)
{
...
double windturbine_serverProxy::windturb_dgsync(double pCircuit_V_A_Re, double pCircuit_V_A_Im, double pCircuit_V_B_Re, double pCircuit_V_B_Im, double pCircuit_V_C_Re, double pCircuit_V_C_Im)
{
...
int windturbine_serverProxy::windturb_dg_postsync(int t0, int t1)
{
